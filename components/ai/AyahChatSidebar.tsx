...
// ---------------------------------------------------------------------------
// Shared inner content — rendered inside both the Vaul mobile drawer and the
// desktop motion.div sidebar. Props are passed down so each branch can supply
// its own close handler / container ref.
// ---------------------------------------------------------------------------
function ChatContent({
  ...
  return (
    <>
      ...
      {/* Input Area */}
      <div className="relative z-10 shrink-0 bg-zinc-950 border-t border-zinc-800/80 p-3 sm:p-4 pb-[max(env(safe-area-inset-bottom,0px),8px)] w-full">
...
export default function AyahChatSidebar({ surahNumber, ayahNumber, isOpen, onClose, initialModeId, rootWord }: AyahChatSidebarProps) {
  ...
  // Fetch token balance when opening
  useEffect(() => {
    if (isOpen) {
      fetch('/api/ai/rag')
        .then(res => {
  ...
  // Set greeting message when context changes
  useEffect(() => {
    if (isOpen) {
  ...
  return (
    <>
      {/* ------------------------------------------------------------------ */}
      {/* MOBILE: Vaul bottom-sheet drawer (< xl)                             */}
      {/* Vaul owns scroll-lock, keyboard-avoidance, and open/close lifecycle */}
      {/* ------------------------------------------------------------------ */}
      <div className="xl:hidden">
        <Drawer.Root open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}>
          <Drawer.Portal>
            <Drawer.Overlay className="fixed inset-0 bg-zinc-950/80 z-[100]" />
            <Drawer.Content
              className="bg-zinc-950 flex flex-col fixed bottom-0 left-0 right-0 z-[101] max-h-[85dvh] rounded-t-3xl border-t border-emerald-500/30 shadow-[0_-20px_50px_-10px_rgba(16,185,129,0.15)] outline-none"
              aria-label="AI Scholar Chat"
            >
              {/* Decorative top edge glow */}
              <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-emerald-500/40 to-transparent shadow-[0_0_15px_rgba(16,185,129,0.5)] z-50 pointer-events-none rounded-t-3xl" />

              <ChatContent {...sharedContentProps} />
            </Drawer.Content>
          </Drawer.Portal>
        </Drawer.Root>
      </div>
      ...
      {/* ------------------------------------------------------------------ */}
      {/* DESKTOP: Framer Motion right sidebar (≥ xl) — unchanged             */}
      {/* ------------------------------------------------------------------ */}
      <AnimatePresence>
        {isOpen && (
          <>
            ...
            <motion.div
              ...
              exit={{ opacity: 0 }}
              onClick={onClose}
              className="hidden xl:block fixed inset-0 bg-black/20 z-40"
            />
            ...
            <motion.div
              ...
              className="hidden xl:flex fixed z-50 top-0 right-0 h-dvh max-h-dvh w-[450px] bg-zinc-950 flex-col items-start shadow-2xl border-l border-emerald-500/20 shadow-[-20px_0_50px_-10px_rgba(16,185,129,0.15)]"
            >
              {/* Decorative left edge line */}
              <div className="absolute left-0 top-0 bottom-0 w-[2px] bg-gradient-to-b from-transparent via-emerald-500/40 to-transparent shadow-[0_0_10px_rgba(16,185,129,0.5)] z-50 pointer-events-none" />
