...
const CylindricalGallery = forwardRef<CylindricalGalleryHandle, CylindricalGalleryProps>(function CylindricalGallery({
  ...
  const { squeezeMax, squeezeWidth, chromaticAberration, opacity, emission, saturation, brightness, scanLines, scanLineSpeed, scanLineDensity, distanceFadeStart, distanceFadeEnd, flickerIntensity, flickerSpeed } =
    useControls("Effects", {
      ...
      emission: { value: 0.25, min: 0, max: 3, step: 0.05, label: "Emission" },
      saturation: { value: 1.0, min: 0, max: 3, step: 0.05, label: "Saturation" },
      brightness: { value: 1.0, min: 0.2, max: 3, step: 0.05, label: "Brightness" },
      scanLines: { value: 0.4, min: 0, max: 1, step: 0.05, label: "Scan Lines" },
      ...
      flickerIntensity: { value: 0.10, min: 0, max: 1, step: 0.01, label: "Flicker" },
  ...
  useFrame((state, delta) => {
    ...
    if (textGroupRef.current) {
      ...
      for (let i = 0; i < NUM_INSTANCES; i++) {
        if (texts[i]) {
          const uScrollOffset = scrollOffset.current * scrollAdvanceSpeed;
          let scrolledY = positionYsRef.current[i] + uScrollOffset;
          scrolledY = ((scrolledY + totalHeight * 0.5) % totalHeight + totalHeight) % totalHeight - totalHeight * 0.5;

          const y = scrolledY;
          const squeezeGauss = Math.exp(-(y * y) / (squeezeWidth * squeezeWidth));
          const squeezedRadius = radius * (1.0 - smoothSqueeze.current * squeezeGauss);
          
          const angle = angleOffsetsRef.current[i] + rotation.current;
          
          const distFade = 1.0 - THREE.MathUtils.smoothstep(Math.abs(y), distanceFadeStart, distanceFadeEnd);
          
          const x = Math.sin(angle) * (squeezedRadius + 0.1);
          const z = Math.cos(angle) * (squeezedRadius + 0.1);

          texts[i].position.set(x, y + 1.2, z);
          texts[i].rotation.y = angle;
          
          // Hide if too far back or faded out
          texts[i].visible = !isDebug && Math.cos(angle) > 0 && distFade > 0.05;
          // Scale based on fade
          texts[i].scale.setScalar(distFade * imageScale);
        }
      }
    }
  });
