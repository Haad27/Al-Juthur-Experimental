...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!responseText) {
      console.log("Gemini failed or ran out of quota. Falling back to OpenRouter...");
        ...
        try {
          const orPayload = {
            ...
            messages: [
              {
                role: "user",
                content: `${TRANSLATION_PROMPT}\n\nTranslate the following text strictly according to the rules. ALWAYS RETURN A RAW JSON ARRAY. DO NOT WRAP IN \`\`\`json \`\`\`. DO NOT ADD ANY OTHER TEXT.\n\n${text}`
              }
            ]
          };
          ...
          if (orRes.ok) {
            ...
            console.warn("OpenRouter API Error:", await orRes.text());
          }
        } catch(e: any) {
      ...
      if (!responseText) {
        throw new Error(`Failed to generate translation from Gemini and OpenRouter. Last error: ${lastError}`);
      }
    }
