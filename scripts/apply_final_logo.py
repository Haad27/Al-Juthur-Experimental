...
def extract_large_arabic_calligraphy(source_path):
    ...
    # We only take the Arabic calligraphy part (y from 0 to 478 inside the crop), excluding the tiny English subtext
    # which made the icon tiny in small sizes
    ...
    print('Extracted large, pure Arabic calligraphy:', art_png.size)
    return art_png

def generate_rounded_app_icon(art, size, icon_ratio=0.84, corner_ratio=0.20):
    scale = 4
    high_size = size * scale
    
    # 1. Emerald green canvas with rounded corners (#10b981)
    ...
    # 2. Place large, bold Arabic calligraphy in center with tight balanced padding
...
def generate_og_share_image(art, width=1200, height=630, icon_ratio=0.65):
...
def main():
    ...
    # 1. Generate rounded app icons & favicons with large, prominent calligraphy (icon_ratio=0.84)
    app_icons = [
        ('public/assets/favicon/apple-touch-icon.png', 180, 0.84, 0.20),
        ('public/assets/favicon/android-chrome-192x192.png', 192, 0.84, 0.20),
        ('public/assets/favicon/android-chrome-512x512.png', 512, 0.84, 0.20),
    ...
    for dest_path, sz, ir, cr in app_icons:
        ...
        print(f"Generated {dest_path} ({sz}x{sz}) with large, prominent calligraphy.")

    # 2. Multi-resolution favicon.ico
    ico_16 = generate_rounded_app_icon(art, 16, 0.85, 0.20)
    ico_32 = generate_rounded_app_icon(art, 32, 0.85, 0.20)
    ico_48 = generate_rounded_app_icon(art, 48, 0.85, 0.20)
    ...
    og_banner = generate_og_share_image(art, 1200, 630, 0.65)
    og_banner.save('public/og-image.png', 'PNG', optimize=True)
    og_banner.save('public/assets/images/og-image.png', 'PNG', optimize=True)

    og_sq = generate_rounded_app_icon(art, 512, 0.84, 0.0) # Full square for OG
