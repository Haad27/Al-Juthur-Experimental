...
async function runAll() {
  try {
    await migrateFile('database/lexicon/data/arabic_lexicons.sqlite');
    await migrateFile('database/lexicon/data/quran.db');
    await migrateFile('database/lexicon/data/english_morphology.sqlite');
    
    console.log("\n✅ ALL DONE! Lexicon Migration to Turso successful!");
    process.exit(0);
  } catch (err) {
    console.error(err);
    process.exit(1);
